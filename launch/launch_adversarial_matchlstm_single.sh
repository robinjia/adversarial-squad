...
cl run data.json:${dataset} nearby.json:${nearby_file} :src-addwords :nltk_data english.txt:brown_english_1k.txt :glove_shortlists :eval_squad.py :matchlstm :ivocab_t.t7 :vocab_t.t7 :initEmb_t.t7 :js2tokens.py :txt2js.py :model_BE_1 :model_BE_2 :model_BE_3 :model_BE_4 :model_BE_5 :model_bpoint :ensemble.lua :utils.lua :Embedding.lua :CAddRepTable.lua :LSTM2.lua :LSTMwwatten2.lua :pointNet4.lua :pointBEMlstm2.lua :bpointBEMlstm.lua :main4.lua :bash.sh "python src-addwords/adversarial_squad.py ${mode} matchlstm-single data.json english.txt -n ${num_additions} -k ${sample_num} -p ${num_particles} -q ${num_nearby} -T ${num_epochs} --nearby-file nearby.json -o adversarial_data.json --plot-file eval.png; th main4.lua -input adversarial_data.json; python eval_squad.py adversarial_data.json prediction.json -o eval.json; rm -rf adversarial_squad_out" --request-docker-image robinjia/shuohang-torch-1.8:v1.1 -n adv -d "${mode}, MatchLSTM single, n = ${num_additions}, k = ${sample_num}, p = ${num_particles}, q = ${num_nearby}, T = ${num_epochs}, ${dataset}" --request-cpus 1 --request-queue john
